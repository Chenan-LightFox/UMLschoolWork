...
#### 4.5 操作：出钞（对应 `CashDispenser.dispense`）
描述：仅在取款成功后执行出钞。
...
#### 4.6 操作：凭条打印选择（对应 `Display.readReceiptChoice` + `ReceiptPrinter.print`）
描述：用户可选择是否打印取款凭条。
...
#### 4.7 操作：结束会话（对应 `Display.showFarewell`）
描述：交易完成后提示用户取卡离开。
