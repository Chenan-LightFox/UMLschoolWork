...
### 五、补充规约（Supplementary Specification）
...
### 七、迭代计划（Iteration Plan）

#### 7.1. 初始阶段（Inception）
- 完成 Vision 文档
- 确定主要用例

#### 7.2. 细化迭代1（Iteration 1）
...
#### 7.3. 细化迭代2（Iteration 2）
- 完成查询余额、修改密码
- 完善用例模型

#### 7.4. 细化迭代3（Iteration 3）
- 完成转账、存款功能
- 完善系统模型
...
##### 8.11.5 操作：出钞（对应 `CashDispenser.dispense`）
描述：仅在取款成功后执行出钞。
...
##### 8.11.6 操作：凭条打印选择（对应 `Display.readReceiptChoice` + `ReceiptPrinter.print`）
描述：用户可选择是否打印取款凭条。
...
##### 8.11.7 操作：结束会话（对应 `Display.showFarewell`）
描述：交易完成后提示用户取卡离开。
...
#### 9.1. 设计类图（Design Class Diagram）

#### 9.2. 详细顺序图（Sequence Diagram）

#### 9.3. 方法设计（Design Class）
